...
export function updApiStatus(){
  if(App.transcriptionMode==='online'&&App.dgKey){
    D.dgStatus.className='api-key-status connected';D.dgStatusText.textContent='Online — API key saved';updConn('connected','Online — connected');
  }else if(App.transcriptionMode==='online'&&!App.dgKey){
    D.dgStatus.className='api-key-status disconnected';D.dgStatusText.textContent='No API key configured';updConn('disconnected','No API key configured');
  }else{
