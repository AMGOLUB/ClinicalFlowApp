...
> **Updated:** 2026-03-06 (demo system, dictionary features, settings reorg, ASR corrections expansion)
> **Version:** 1.0.0
...
13. [Dental Charting System](#13-dental-charting-system)
   - 13.1 [Demo System (4 Scenarios)](#131-demo-system-4-scenarios)
   - 13.2 [Dictionary Features Module](#132-dictionary-features-module)
   - 13.3 [Settings Reorganization](#133-settings-reorganization)
...
│  │            MARKETING WEBSITE                        │   │
│  │            clinicalflow.us                          │   │
│  │  12 HTML pages + CSS + JS + screenshots             │   │
...
│   ├── dictionary-features.js           # Dictionary tooltips, phrase palette, autocomplete (~530 lines)
...
│   ├── corrections.json                # English medical corrections (407+ entries)
...
│   │   ├── corrections.json            # Bundled English corrections (407+ entries)
│   │   ├── corrections-{de,es,fr,it,pt}.json
│   │   ├── medical-dictionary.json     # Medical term definitions for tooltips
│   │   ├── dental-dictionary.json      # Dental term definitions for tooltips
│   │   ├── radiology-dictionary.json   # Radiology term definitions for tooltips
│   │   └── fonts/                      # Custom fonts
...
│   ├── feedback.html                   # Feedback form (12KB)
...
| **storage** | `storage.rs` | ~500+ | File I/O for auth.json, config.json (encrypted), session.json (encrypted), session archives, PDF export (delegates to pdf module), corrections loading, dictionary loading (`load_dictionary` command), log cleanup |
...
|--------|---------|----------|
| `app.js` | Entry point, initialization, coordinator | `init()`, `_initTauri()`, demo system (4 scenarios), `DEMO_MEDICAL`/`DEMO_DENTAL`/`DEMO_RADIOLOGY`/`DEMO_BEHAVIORAL` constants |
...
| `settings.js` | Settings drawer UI (General + Advanced tabs) | API key inputs, engine selection, device picker, grouped settings (Notes, Recording, Export Actions, Dental, EHR, PMS, Language, Security) |
...
| `dictionary-features.js` | Dictionary tooltips, phrase palette, autocomplete | Lazy-loads 3 dictionary JSONs, builds lookup Maps, tooltip delegation, Ctrl+J palette, 200ms debounce autocomplete, tooth number highlighting |
...
| `templates.js` | Note templates (22 built-in) | SOAP, HPI, DAP, BIRP, specialty, radiology, dental, custom |
...
| `corrections.json` (x6) | EN: ~23 KB, others ~2 KB each | Phonetic error correction dictionaries (EN 407+ entries, DE, ES, FR, IT, PT) |
| `medical-dictionary.json` | — | Medical term definitions and abbreviations for dictionary features |
| `dental-dictionary.json` | — | Dental term definitions for dictionary features |
| `radiology-dictionary.json` | — | Radiology term definitions for dictionary features |
...
| `feedback.html` | 12KB | No | Yes (edge fn) | Bug reports, feature requests, general feedback |
...
- 407+ English patterns (expanded from original 82) + 122 multilingual patterns
- Regex-based phonetic error correction
- Coverage: medications, conditions, procedures, dental terms, radiology modalities, contrast agents, IR procedures, nuclear medicine, findings, anatomy
- Examples: "hyper tension" → "hypertension", "peri odontal" → "periodontal", "metro nida zole" → "metronidazole", "fluoro scopy" → "fluoroscopy"
- Applied in real-time to each transcript chunk
- Both `src-tauri/resources/corrections.json` and `src/corrections.json` kept in sync
...
### 12.4 Line-Level Editing & Dictation

**Module:** `notes.js`

After note generation, each line renders with inline controls (move up/down, add, delete, drag handle, mic). Lines can be dragged across sections (e.g., Subjective → Objective). An "Edit" button per section toggles full-text contentEditable mode. All line-level controls are gated behind the `noteLineActions` setting (default: true).

**Line Dictation** (enabled via `noteLineActions` setting — the former `lineDictation` setting was merged into `noteLineActions`):

| Aspect | Detail |
|--------|--------|
| **Engine** | Deepgram `nova-3-medical` streaming (not Whisper — speed is critical) |
| **Audio pipeline** | cpal `stream` mode → `audio-pcm` Tauri events → Deepgram WebSocket |
| **Punctuation** | `smart_format` and `punctuate` disabled; spoken punctuation converted to symbols ("period" → `.`) |
| **Casing** | Lowercase by default, auto-capitalized after sentence-ending punctuation |
| **Medical terms** | Same `MED_RX` keyterms + `CORRECTIONS_DICT` + `applyLiveCorrections()` as main transcription |
| **Cursor-aware** | Inserts at click position; click to reposition during active dictation; highlight text to replace |
| **Constraint** | Cannot run during active patient recording (cpal single-stream limitation) |
...
### 13.1 Demo System (4 Scenarios)

**Module:** `app.js` — demo data constants and playback functions

The demo system allows users to explore all app features without recording. Triggered by triple-clicking the help button, which opens a demo picker modal.

| Scenario | Constant | Playback | Template | Details |
|----------|----------|----------|----------|---------|
| **General Medical** | `DEMO_MEDICAL` | Animated (`runDemoAnimated`) | SOAP (default) | Multi-speaker transcript with word-by-word animation |
| **Dental Examination** | `DEMO_DENTAL` | Instant (`runDemoInstant`) | General dental exam | Auto-populates dental chart + perio data |
| **Radiology Report** | `DEMO_RADIOLOGY` | Instant (`runDemoInstant`) | `radiology_diagnostic` (auto-selected) | Single-speaker dictation by Dr. Morrison |
| **Behavioral Health** | `DEMO_BEHAVIORAL` | Instant (`runDemoInstant`) | `dap` (auto-selected) | Dr. Kim + patient Jessica, multi-speaker |

**Key functions:**
- `runDemoAnimated(data)` — Simulates live transcription with word-by-word animation
- `runDemoInstant(data)` — Loads transcript and chart data immediately
- `_setDemoTemplate(fmt)` — Sets `App.noteFormat`, updates config, and syncs the format selector dropdown

### 13.2 Dictionary Features Module

**Module:** `src/dictionary-features.js` (~530 lines)
**Setting:** `dictionaryFeatures` (default: `false`)
**CSS:** 181 lines added to `src/styles.css` (tooltips, phrase palette, autocomplete, tooth badges)

All features gated behind `App.settings.dictionaryFeatures` toggle in Settings → Advanced → Notes.

#### Data Loading

Lazy-loads 3 dictionary JSON files via Tauri `load_dictionary` command or `fetch()` fallback:
- `medical-dictionary.json` — General medical terms, abbreviations, scoring systems
- `dental-dictionary.json` — Dental terminology, tooth anatomy, procedures
- `radiology-dictionary.json` — Imaging modalities, findings, anatomy

Builds 4 lookup Maps on first load:
| Map | Contents |
|-----|----------|
| `_abbrMap` | Abbreviation → full term expansions |
| `_medMap` | Medical/dental/radiology term definitions |
| `_scoringMap` | Clinical scoring system references |
| `_toothMap` | Tooth number → name/position mappings |

#### Features

| Feature | Trigger | Detail |
|---------|---------|--------|
| **Tooltips** | Hover on recognized term | Event delegation on transcript/note containers; shows definition popup |
| **Phrase Palette** | `Ctrl+J` | Template-aware phrase groups; inserts at cursor or copies to clipboard |
| **Autocomplete** | Typing 3+ chars | 200ms debounce, up to 8 suggestions from template-specific term lists |
| **Tooth Highlighting** | Auto on render | TreeWalker finds `#N` patterns, wraps in `.tooth-number-ref` spans |

#### Rust Backend

`load_dictionary` command in `storage.rs` loads dictionary JSON files from the bundled resources directory. Dictionaries included via `tauri.conf.json` resources glob pattern.

### 13.3 Settings Reorganization

The settings drawer is organized into two tabs:
...
| **AI & Transcription** | API keys, engine selection, model picker |

**Advanced Tab:**
| Group | Settings |
|-------|----------|
| **Notes** | `autoCoding` (auto ICD/CPT), `noteLineActions` (line move/add/delete/drag/mic), `dictionaryFeatures` (tooltips/palette/autocomplete) |
| **Recording** | `autoScroll`, `timestamps`, `autoDetect`, `highlightTerms` |
| **Export Actions** | `showCopyEhr`, `showExportHl7`, `showNarrative`, `showSyncPms` |
| **Dental** | Conditional — only shown when dental template active |
| **EHR Integration** | Chrome extension configuration |
| **PMS Bridge** | Open Dental connection settings |
| **Language** | Transcription language selection |
| **Security** | PIN, auto-lock timer |

**Setting changes (v1.0.0 → current):**
- `lineDictation` — **removed**, merged into `noteLineActions` (controls move/add/delete/drag/mic)
- `dictionaryFeatures` — **added** (default: `false`)
- `noteLineActions` — **added** (default: `true`)
...
### Actual Credential Values (THIS FILE IS GITIGNORED — NEVER COMMIT)

#### Supabase
| Key | Value |
|-----|-------|
| **Project URL** | `https://seuinmmslazvibotoupm.supabase.co` |
| **Project ID** | `seuinmmslazvibotoupm` |
| **Anon Key** | `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNldWlubW1zbGF6dmlib3RvdXBtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIyMjM4MjAsImV4cCI6MjA4Nzc5OTgyMH0.YE4e8bf6Q_4zfvu9zLpaMIrfbnsf6Z9ucf2mSebyrqY` |
| **Service Role Key** | *(retrieve from Supabase Dashboard → Settings → API)* |
| **Dashboard** | `https://supabase.com/dashboard/project/seuinmmslazvibotoupm` |

#### Cloudflare R2
| Key | Value |
|-----|-------|
| **S3 Endpoint** | `https://97a50e2504ab5379203c12ac0ba7964b.r2.cloudflarestorage.com` |
| **Access Key ID** | `86a138f75c3b424ae1abd451418c1277` |
| **Secret Access Key** | `8cc5113623ba54a59462e1c3df2aca72086842576d1260a79e18fb8182489d00` |
| **Bucket** | `clinicalflow-releases` |
| **Local CLI profile** | `~/.aws/credentials` → `[r2]` profile |
| **Upload command** | `aws s3 cp <file> s3://clinicalflow-releases/<key> --endpoint-url https://97a50e2504ab5379203c12ac0ba7964b.r2.cloudflarestorage.com --region auto --profile r2` |

#### Compiled-In Keys (in src-tauri/src/license.rs)
| Key | Purpose | Value |
|-----|---------|-------|
| **SESSION_KEY** | Encrypts session.json (auth tokens, not PHI) | `\x57\x07\xbe\xdb\x00\x38\xe5\x51\xa7\x5b\x8a\xb4\xbf\x0f\xcf\xb6\xb4\x57\x7c\x7e\xfc\x6e\xa0\x88\xbb\x79\xd0\x90\xfe\x79\xb3\x96` |
| **LICENSE_KEY** | Decrypts server-issued license blobs (must match `LICENSE_ENCRYPTION_KEY` on Supabase) | `\x6c\xaa\x76\xa3\xb2\x7b\x54\xe6\x29\x2b\xbe\xb9\xc8\xe5\x09\x03\xc6\x4c\x62\xcc\xdf\x4b\x8a\xb8\x45\x44\x85\x44\x11\xde\x07\xa9` |

#### Stripe (NOT YET LIVE — set these when ready)
| Key | Value |
|-----|-------|
| **Secret Key** | `sk_live_...` *(create at dashboard.stripe.com)* |
| **Webhook Secret** | `whsec_...` *(created when adding webhook endpoint)* |
| **Price IDs** | *(created when adding products — see section above)* |
...
Hosted on **GitHub Pages** via `.github/workflows/deploy-website.yml`. Auto-deploys on push to `main` when `ClinicalFlowWebsite/**` changes. CNAME file in `ClinicalFlowWebsite/CNAME` points to `clinicalflow.us`.

All pages load shared `main.js` (nav scroll, mobile hamburger menu, scroll-reveal, platform detection) and `auth-nav.js` (auth-aware nav state).
...
| **S3 Endpoint** | `https://97a50e2504ab5379203c12ac0ba7964b.r2.cloudflarestorage.com` |
| **Files** | `ClinicalFlow_1.0.0_aarch64.dmg` (macOS), `ClinicalFlow_1.0.0_x64-setup.exe` (Windows) |
| **Access** | Via presigned URLs (15-min expiry) from `download-release` edge function |
| **Access Key ID** | `86a138f75c3b424ae1abd451418c1277` |
| **Secret Access Key** | `8cc5113623ba54a59462e1c3df2aca72086842576d1260a79e18fb8182489d00` |
| **Local CLI profile** | `~/.aws/credentials` → `[r2]` profile |
| **Upload command** | `aws s3 cp <file> s3://clinicalflow-releases/<key> --endpoint-url https://97a50e2504ab5379203c12ac0ba7964b.r2.cloudflarestorage.com --region auto --profile r2` |
...
|-----------|-------|---------|
| `src/` | ~26 | Desktop app frontend (vanilla JS modules, incl. dictionary-features.js) |
...
| `src-tauri/resources/` | ~11 | Models, corrections dictionaries (x6), dictionary JSONs (x3), fonts |
| `src-tauri/icons/` | 5 | App icons (png, icns, ico) |
| `ClinicalFlowWebsite/` | ~23 | Marketing website (12 HTML pages + CSS + JS + SEO) |
...
**Changes since last doc update (2026-03-02 → 2026-03-06):**
- Demo system expanded from 2 to 4 scenarios (added Radiology Report + Behavioral Health)
- `dictionary-features.js` — new module (~530 lines): tooltips, phrase palette (Ctrl+J), autocomplete, tooth highlighting
- 3 dictionary JSON files added as bundled resources (medical, dental, radiology)
- `corrections.json` expanded from 82 to 407+ entries (medications, dental, radiology, anatomy)
- Settings drawer reorganized into General + Advanced tabs with grouped categories
- `lineDictation` setting removed, merged into `noteLineActions`
- `dictionaryFeatures` and `noteLineActions` settings added
- 181 lines added to `src/styles.css` for dictionary feature UI
- `load_dictionary` Tauri command added to `storage.rs`

**Earlier commits (initial release → 2026-03-02):**
